...
int *same_color_sum,*same_color_sum_dev;
...
{//初始化为所有还没有染色的顶点的颜色都是不合法的
    ...
    {
        dev_color_unvalid[tid] = 1;
        dev_undone[tid] = 1;
        dev_remain_ver[tid] = tid;
...
{
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    int NeighborColors_MAX = 0;
    int cur_ver;
    int GRANT = blockDim.x*gridDim.x;
    int beg_adj_inx,end_adj_inx;
	*dev_changed = false;
    while (tid < remain_ver_sum) 
    { 
        cur_ver = dev_remain_ver[tid];
        ...
        {//该点颜色没有确定
            ...
            {//获取邻居点的颜色,出度
                ...
                if(dev_colors[adj_ver] > NeighborColors_MAX)
                    NeighborColors_MAX = dev_colors[adj_ver];
                beg_adj_inx++;
            }
            beg_adj_inx = dev_col_ptr[cur_ver];
            end_adj_inx = dev_col_ptr[cur_ver+1];
            ...
            dev_colors[cur_ver] = NeighborColors_MAX +1+(flag*threadIdx.x)%2;
                    //有顶点状态发生了改变
			*dev_changed = true;
        } 
        tid += GRANT;
    ...
    while(tid < remain_ver_sum)
    {
        ...
        {//该点颜色还未确定合适	
            ...
            while(beg_adj_inx<end_adj_inx)
            {
                ...
                if (NeighborColor == dev_colors[cur_ver])
                {//同色只变小号的颜色
...
)
{
    ...
    color_unvalid = (int *)malloc(sizeof(int) * n_vertices);
    memset(color_unvalid, 0, sizeof(int) * n_vertices);
    memset(colors, 0, sizeof(int) * n_vertices);
    ...
    cudaMalloc(&same_color_sum_dev,sizeof(int));
              ...
              int iter2)
{
    ...
    while(continue_flag) 
    {
        ...
        if (cudaSuccess != err)
        {
            fprintf(stderr, "1243：getLastCudaError() CUDA error: %s\n", cudaGetErrorString(err));
            exit(EXIT_FAILURE);
        }
        ...
        fprintf(fp,"first:lun=%d undone=%d\n",tmp_lun,num_undone);
        tmp_lun++;
        gettimeofday(&end_time, NULL);
    ...
    {
        gettimeofday(&start_time, NULL);
        remain_ver_sum = get_remain_ver(n_vertices,remain_ver,undone,color_unvalid);//4毫秒
        int nt = 256;
        int nb = (remain_ver_sum + nt - 1)/nt; 
        nb = nb > MAX_GRID_SIZE ? MAX_GRID_SIZE : nb;
        cudaMemcpy(dev_remain_ver,remain_ver,sizeof(int) * n_vertices,cudaMemcpyHostToDevice);
        cudaMemcpy(dev_color_unvalid,color_unvalid,sizeof(int) * n_vertices,cudaMemcpyHostToDevice);
        ...
        {//大约需要00+轮
            ...
            //{
                cudaMemcpy(undone,dev_undone,sizeof(int) * n_vertices,cudaMemcpyDeviceToHost);
                int before_num_undone = std::count(undone, undone + n_vertices, 1);
                remain_color_ColorGraph<<<nb, nt>>>(
                    ...
                    0);
                //cudaMemcpy(undone, dev_undone, sizeof(int) * n_vertices, cudaMemcpyDeviceToHost);
                //int old_num_undone = std::count(undone, undone + n_vertices, 1);
                ...
                if (cudaSuccess != err)
                {
                    fprintf(stderr, "1297：getLastCudaError() CUDA error: %s\n", cudaGetErrorString(err));
                    exit(EXIT_FAILURE);
                }
                cudaMemcpy(undone,dev_undone,sizeof(int) * n_vertices,cudaMemcpyDeviceToHost);
                int after_num_undone = std::count(undone, undone + n_vertices, 1);
                fprintf(fp,"second:lun= %d %d %d %d\n",tmp_lun,before_num_undone,before_num_undone - after_num_undone,after_num_undone);
                tmp_lun++;
                //cudaMemcpy(changed, dev_changed, sizeof(bool), cudaMemcpyDeviceToHost);
                //cudaMemcpy(undone, dev_undone, sizeof(int) * n_vertices, cudaMemcpyDeviceToHost);
                //int new_num_undone = std::count(undone, undone + n_vertices, 1);
                //printf("%d : same_color_sum = %d\n",lun,new_num_undone-old_num_undone);
            //}
            //printf("lun = %d\n",lun);
        } 
...
void mixColor_2(const int n_vertices, 
              const int n_edges,  
              const int *row_ptr, 
              const int *col, 
              const int *col_ptr, 
              ...
              int iter2)
{
    ...
    remain_color_init<<<nb,nt>>>(n_vertices,dev_remain_ver,dev_undone,dev_color_unvalid);//4毫秒
    int lun = 0;
    FILE *fp;
    fp = fopen("./result.txt","w");
    int flag =0;
    ...
    {//大约需要00+轮
        ...
        while(*changed == true)
        {
            cudaMemcpy(undone,dev_undone,sizeof(int) * n_vertices,cudaMemcpyDeviceToHost);
            int before_num_undone = std::count(undone, undone + n_vertices, 1);
            float change_add = (before_num_undone*1.0)/n_vertices;
            if(flag==0 &&  change_add < 0.0001)
                flag = 1;
            remain_color_ColorGraph<<<nb, nt>>>(
                remain_ver_sum, 
                dev_remain_ver,
            ...
            cudaMemcpy(undone,dev_undone,sizeof(int) * n_vertices,cudaMemcpyDeviceToHost);
            int after_num_undone = std::count(undone, undone + n_vertices, 1);
            fprintf(fp,"%d %d %d\n",before_num_undone,before_num_undone - after_num_undone,after_num_undone);
            cudaMemcpy(changed, dev_changed, sizeof(bool), cudaMemcpyDeviceToHost);
            lun++;
        }
        fclose(fp);
...
{
    cudaFree(dev_continue_flag);
    cudaFree(dev_remain_ver);
    cudaFree(dev_srcs);
    cudaFree(dev_dsts);
    cudaFree(dev_colors);
    cudaFree(dev_undone);
    cudaFree(dev_undone_2);
    cudaFree(dev_col);
    cudaFree(dev_col_ptr);
    cudaFree(dev_row);
    cudaFree(dev_row_ptr);
    cudaFree(dev_color_unvalid);
