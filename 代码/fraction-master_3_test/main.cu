...
void test(const char* filename, const int start_index, const int max_iters, const float fraction, int deviceNum, float iter2)
{
...
//         << (isRight(num_vertices, row_ptr, col, col_ptr, row, colors) ? "right solution" : "wrong solution") << endl << endl;
   ...
   // cout << "Running mix coloring algorithm (Run " << max_iters << "iters on GPU)...\n";
    ...
    if(iter2 > 0)
        ...
        mixColor_2(num_vertices, num_edges, row_ptr, col, col_ptr, row, max_iters, fraction, colors,iter2);
    gettimeofday(&end_time, NULL);
    after_mix_color();
...
int main(int argc, char ** argv)
{
    ...
    /***********************************************************************
      test(filename.c_str(), start_index, max_iters, fraction, deviceNum);
    ************************************************************************/
    for (fraction = 0.05; fraction <= 1.03; fraction+=0.05)
        for (int i =0; i < 3; i++)
