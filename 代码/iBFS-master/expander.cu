...
)
{
	...
	while(vec_id<q_sz)
	{
		...
		while(lane<end_curr)
		{//一个block一次处理一个边界点，用block内部的线程来处理其边
			adj_vert=adj_list_d[lane];//邻接点
			adj_depth=depth_comp_curr[adj_vert];//邻接点深度
			if((~(adj_depth.x))&vert_depth.x)//当且仅当其状态为0，而边界点状态为1时，成立
				atomicOr(&(depth_comp_curr[adj_vert].x),vert_depth.x);
...
)
{
	...
	while(vec_id<q_sz)
	{
		ex_ver_curr	= ex_q_d[vec_id];//联合的边界队列
		beg_curr		= beg_pos_d[ex_ver_curr];
		end_curr		= beg_pos_d[ex_ver_curr+1];
		vert_depth	= depth_comp_last[ex_ver_curr];//读取联合状态
...
)
{
	...
	while(tid<q_sz)
	{
		...
		while(lane<end_curr)
		{
			if((~vert_depth)==0) break;//在每一个并发中都被访问了
			adj_depth_curr=depth_comp_last[adj_list_d[lane]];
			if(((~vert_depth)&adj_depth_curr) != 0) 
				vert_depth|=adj_depth_curr;		//与每一个可能改变其状态的点都进行或操作	
			lane++;
		}

		if((vert_depth^depth_comp_curr[ex_ver_curr]) != 0) 
			depth_comp_curr[ex_ver_curr]=vert_depth;//状态改变
		tid += GRNLTY;
	}
}
...
{
	if(level<=sw_level)	//切换层
		td_expand(level);
	else if((level==sw_level+1))
		sw_expand(level);
	else{//也使用合并的fq
		//differ sw_expand of no imbalance issue不同的sw_expand，没有不平衡问题
		bu_expand(level);
	}
}
