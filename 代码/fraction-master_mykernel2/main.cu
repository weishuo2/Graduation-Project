...
void test(const char* filename, const int start_index, const int max_iters, const float fraction, int deviceNum,float fraction2)
{
...
//         << (isRight(num_vertices, row_ptr, col, col_ptr, row, colors) ? "right solution" : "wrong solution") << endl << endl;
   ...
   // cout << "Running mix coloring algorithm (Run " << max_iters << "iters on GPU)...\n";
    ...
    gettimeofday(&start_time, NULL);
    mixColor(num_vertices, num_edges, row_ptr, col, col_ptr, row, max_iters, fraction, colors,fraction2);
    gettimeofday(&end_time, NULL);
    after_mix_color();
...
int main(int argc, char ** argv)
{
    ...
    if (argc < 3)
    {
      cout << "Usage: " << argv[0] << " --file filename [--start_index number] [--max_iters max_ters] [--frac fraction_value] --dev deviceNum [--frac2 fraction_value]" << endl;
        return 0;
    }
    string filename = "../data/web-Stanford.txt";
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        if (strcmp(argv[i], "--frac2") == 0)    
            fraction2 = atof(argv[i+1]); 
    }
    ...
    ************************************************************************/
    ...
    for (int i =0; i < 10; i++)
      test(filename.c_str(), start_index, max_iters, fraction, deviceNum,fraction2);
