...
)
{
	...
	*dev_changed = false;
    ...
    { 
        ...
        {//该点颜色没有确定
            int NumNeighbors = 0;
            beg_adj_inx = dev_row_ptr[tid];
            end_adj_inx = dev_row_ptr[tid+1];
            while(beg_adj_inx<end_adj_inx)
            {//获取邻居点的颜色,出度
    ...
    if(tid < n_vertices)
    {
        ...
        {//该点颜色还未确定合适	
            ...
            while(beg_adj_inx<end_adj_inx)
            {
                ...
                if (NeighborColor == dev_colors[tid] && tid < dev_col[beg_adj_inx])
                {//同色只变小号的颜色
		...
		// Update the vertex's coloring status
		    dev_color_unvalid[tid] = ColorUnValid;
        }
	}
}
