...
inline off_t fsize(const char *filename) {
    struct stat st; //获取对应的文件信息
    if (stat(filename, &st) == 0)
        return st.st_size;//文件对应的字节数
    return -1; 
}
...
int main(int argc, char** argv){
	...
	size_t file_size = fsize(argv[1]);//文件大小
	...
	//参数依次为，映射内存起始地址（NULL为让系统选），第二个为映射多大，第三个为映射区域的保护方式，第四个为文件指针，第五个不知
	//返回的是在内存中的起始地址
	assert(ss_head != MAP_FAILED);//判断是否成功
	...
	file_size -= head_offset;//重新设置开始地点和文件大小
	...
	size_t edge_count=0;
	size_t vert_count;
	vertex_t v_max = 0;//记录编号最小和最大顶点的编号
	...
	if(!is_reverse) edge_count >>=1;//判断是否为有向图
	...
	ftruncate(fd4, edge_count*sizeof(vertex_t));//定义文件大小
	...
	while(offset<line_count){
		...
		char* sss1=ss+curr;
		dest=atol(sss1)-v_min;//修正为顶点是从0开始的
...
//	exit(-1);
	...
	for(size_t i=1; i<vert_count; i++){//计算每一个顶点第一个出边在CSR中的索引
...
//	for(size_t i=0; i<vert_count; i++){
	for(size_t i=0; i<(vert_count<8?vert_count:8); i++){
		cout<<i<<" "<<begin[i+1]-begin[i]<<" ";//输出几个查看一下
