...
new_vert_t,new_index_t,new_weight_t>//点明其中用到的类型变量
::graph(
		const char *beg_file,
		const char *csr_file,
		const char *weight_file)
{
	...
	vert_count=fsize(beg_file)/sizeof(file_index_t) - 1;//与转化代码是对应的，那里多申请了一个空间
	...
	if(file!=NULL)
	{
		...
		//申请对应大小的内存，并且保证首地址为32的倍数，首地址在tmp_csr中
		//类似于malloc
